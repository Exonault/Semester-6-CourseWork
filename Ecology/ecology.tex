...
\end{gather*}
Изследвайте вида на особенните точки,  фазов портрет, компютърна реализация, 
...
\newpage
\section{Решение}
\subsection{Особени точки}
Oсобените точки се получават като решение на системата
\begin{gather*}
		\begin{array}{|l@{}}
...
\begin{gather*}
	...
	\begin{pmatrix} 
		a \\ c
	\end{pmatrix} \implies 
\Delta = \begin{vmatrix}b & \sigma \\ \nu & d \end{vmatrix} \quad 
\Delta_1 = \begin{vmatrix}a & \sigma \\ c & d \end{vmatrix} \quad 
\Delta_2 = \begin{vmatrix}b & a\\ \nu & c \end{vmatrix} \implies \\ \\
	\begin{array}{|l@{}}\tag{$IV$}
...
\begin{gather*}
 \det(W-\lambda I)  = 
	...
	\end{vmatrix} = 
		...
		\end{array} \\
\implies   \lambda_{1,2} = \alpha \pm \beta i \in \mathbb{C}, \alpha < 0 \implies \text{ устойчив фокус} \tag{$IV$} \\
a_{21} = \nu\Delta_2 > 0 \implies \text{ Посока: срещу часовника}
\end{gather*}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\paperwidth]{fazof-portret.jpg}
  \caption{Фазов портрет на обобщения модел}
\end{figure}
...
\begin{itemize}    
	\item a, b, sigma, c, nu, d - коефициенти определящи растежа на популацията. 
	\item N0 - началните стойности на популациите във формата $[N_1(0), N_2(0)]$
	\item i - номер на графиката, която ще бъде създадена
\end{itemize}
Системата уравнения се задават в функцията ode, която приема два аргумента: време t и вектор N, който съдържа стойностите на популациите на двата вида.
След това се използва вградената MATLAB функция ode45, която решава системата диференциални уравнения, зададена в ode, за интервала на времето tspan, като използва началните стойности $N_0$. 
След като решението е генерирано, функцията визуализира резултатите чрез графика.  
...
\begin{gather*}
	...
	\Delta_1 = \begin{vmatrix}5 & 2 \\ 3 & 6 \end{vmatrix} = 30 - 6 = 24 > 0 \qquad
	\Delta_2 = \begin{vmatrix}2 & 5\\ 1 & 3 \end{vmatrix} = 6-5 = 1 > 0
\end{gather*}
За генериране на графика е нужно да се изпълни следния  код в MATLAB
\begin{verbatim}
a = 5;b = 2;sigma = 2;
c = 3;nu = 1;d = 6;
N0 = [1; 1];
...
\end{verbatim}
Където ecologyCompGraph е функцията представена в \ref{subsec:comp}. 
Генерира следната графика:
\begin{figure}[htp!]
\centering
\includegraphics [width=4in]{ecologyComp_01.eps}
\caption{Графика на получените резултати}
\end{figure}\\
Популациите запазват своя брой константен във времето. 
Тук стойността на $N_1$ е $2.4 = \frac{\Delta_1}{\Delta}$. 
Аналогично за $N_2$ стойността е $0.1 = \frac{\Delta_2}{\Delta}$.\\
Фазовия портрет на системата се генерира със следния код във Wolfram Mathematika.
\begin{verbatim}
StreamPlot[{(5-2*x-2*y)*x, (3-1*x-6*y)*y}, {x,0,3 }, {y,0,1 }]
\end{verbatim}
Тъй като самата функция не може да работи с променливи различни от $x,y$ правим полагане $N_1 = x, N_2 = y$. 
Генерира се фазовия портрет.
\begin{figure}[h]
\centering
...
\begin{gather*}
	...
	\Delta_1 = \begin{vmatrix}1 & 1 \\ 2 & 3 \end{vmatrix} = 3 - 2 = 1 > 0 \qquad
	\Delta_2 = \begin{vmatrix}2 & 1\\ 1 & 2 \end{vmatrix} = 4-1 = 3 > 0
\end{gather*}
За генериране на графика е нужно да се изпълни следния MATLAB код
\begin{verbatim}
a = 1;b = 2;sigma = 1;
c = 2;nu = 1;d = 3;
N0 = [1; 1];
...
\end{verbatim}
Където ecologyCompGraph е функцията представена в \ref{subsec:comp}. 
Генерира се следната графика. \\
\begin{figure}[htp!]
\centering
\includegraphics [width=4in]{ecologyComp_02.eps}
\caption{Графика на получените резултати}
\end{figure}\\
И тук двете популации запазват своя брой константен.
Тук стойността за $N_1$ е $0.2 = \frac{\Delta_1}{\Delta}$. 
Аналогично за $N_2$ стойността е $0.6 = \frac{\Delta_2}{\Delta}$. \\
Фазовия портрет на системата се генерира със следния код във Wolfram Mathematika.
\begin{verbatim}
StreamPlot[{(1-2*x-1*y)*x, (2-1*x-3*y)*y}, {x,0,1 }, {y,0,1 }]
\end{verbatim}
И тук е направено полагането $N_1 = x, N_2 = y$. 
Кодът генерира следния фазов портрет.
\begin{figure}[htp!]
\centering
