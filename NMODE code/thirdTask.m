...
f = zeros(n,1);
for i = 1:n
    if i ~= 1; A(i,i-1) = h/(t(i)) - 1;
    end
    A(i,i) = 2-(2*h^2/t(i)^2);
    if i ~= n; A(i,i+1) = -h/(t(i)) - 1;
    end
    f(i) = -h^2*(cos(t(i))^2 * 1/t(i)^2);
end
% Прилагане на граничните условия
A(1,:) = 0; A(1,1) = 1;
A(n,:) = 0; A(n,n) = 1;
f(1) = f(1) - alpha*(h/(t(1)) - 1);
f(n) = f(n) - beta*(-h/(t(n)) - 1);
...
f = zeros(n,1);
for i = 1:n
    if i ~= 1; A(i,i-1) = h/(2*t(i)) - 1;
    end
    A(i,i) = 2 - (16*h^2)/(t(i)^2);
    if i ~= n; A(i,i+1) = -h/(2*t(i)) - 1;
    end
    f(i) = -h^2*sin(1/t(i)^2);
end
% Прилагане на граничните условия
